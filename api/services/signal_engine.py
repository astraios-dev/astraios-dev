...
def _predict_symbol(closes, highs, lows, volumes, seq_len=32):
    import torch

    feat_matrix = _engineer(closes, highs, lows, volumes)
...
async def generate_signals(symbols=None):
    ...
    def _run():
        ...
        for sym in symbols:
            try:
                resp = client.get_kline(category="linear", symbol=sym, interval="60", limit=100)
                if resp["retCode"] != 0:
                    continue
                ...
                if model_ready:
                    result = _predict_symbol(closes, highs, lows, volumes, _config["seq_len"])
                    if result is None:
                        continue
                    ...
                    if action == "BUY":
                        ...
                        rationale_parts.append(f"no clear directional edge")

                    # Supplement with RSI context
                    rsi_val = _rsi(closes)
                    if rsi_val > 70:
